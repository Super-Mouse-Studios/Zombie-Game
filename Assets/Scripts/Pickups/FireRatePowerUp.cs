...
public class FireRatePowerUp : MonoBehaviour
{
    ...
    {
        float originalFireRate = shooting.baseFireRate;

        shooting.baseFireRate *= fireRateMult; // Increase the fire rate
        Debug.Log("Fire rate increased to: " + shooting.baseFireRate);

        yield return new WaitForSeconds(duration); // Wait for the duration of the power-up

        shooting.baseFireRate = originalFireRate; // Reset the fire rate to its original value
        Debug.Log("Fire rate reset to: " + shooting.baseFireRate);
